...
처음으로 팀원들과 함께 프로젝트를 진행하며, 소통도 개발의 일부분이라는 점을 느끼게 되었습니다. 혼자 개발할 때에는 느끼지 못했던 포인트들이 모두 소통의 일부분이 되면서, 팀 회의는 어떤 방식으로 진행해야 할지, 작업 진행 및 공유는 어떻게 해야 할지 등 서툰 점이 많았던 것 같습니다.

이러한 과정을 겪으며 배운 점이 있다면, 바로 프로젝트 기획서의 중요성입니다. 첫 주차에 프로젝트를 기획할 당시, 개발이 아닌 기획에만 일주일을 투자하는 것이 아깝다고 생각했습니다. 하지만 이내 본격적으로 개발에 착수하자, 기술 명세서는 하나의 프로젝트 지침서로서 역할을 수행했고, API 명세서는 명확할 수록 프론트와 백엔드 간의 작업을 원활하게 만들었습니다. 또한 개발 도중 수정시에는 해당 부분을 함께 공유할 수 있는 공통된 문서가 있어야 이중 작업을 방지할 수 있다는 점도 깨닫게 되었습니다. 기본적이지만 중요한 팀워크의 자세를 배울 수 있었으며, 프로젝트 초반 기획서를 꼼꼼하게 작성하고 팀원들과 충분한 합의를 이루는 것이 중요하다는 점을 느끼게 되었습니다.
